...
const MAX_NUMBER_CALL_GENERATE_TWILLIO_TOKEN = 3;
@Injectable({
  providedIn: 'root'
})
export class CallService {
  ...
  getAllHistoryByAdmin(page=0,pageSize=20) {
    page=page+1;
    return this.http.get<any>(`${environment.apiUrl}/api/backend/call/history?page=` + page +'&limit='+pageSize);
  }
  ...
  public reloadTwilio() {
    if(this.numberGenerate < MAX_NUMBER_CALL_GENERATE_TWILLIO_TOKEN){
      console.log("numberGenerate:"+this.numberGenerate);
      this.numberGenerate++;
