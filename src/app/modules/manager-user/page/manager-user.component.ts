...
export class ManagerUserComponent implements OnInit ,AfterViewInit{
      ...
      if (result) {
        switch (action) {
          ...
          case 'delete-list':
            ...
            break;
          case 'show-recording':
            this.userService.showRecording(id).subscribe(data => {
              this.listUsers.data.map((item, index) => {
                if (item.id == id) {
                  item.canShowRecording = true;
                  this.listUsers.data[index] = item;
                }
              });
              this.listUsers.data = this.listUsers.data;
          ...
          case 'hidden-recording':
            this.userService.hiddenRecording(id).subscribe(data => {
              this.listUsers.data.map((item, index) => {
                if (item.id == id) {
                  item.canShowRecording = false;
                  this.listUsers.data[index] = item;
                  console.log("id:" + id);
                  console.log(item);
                }
  ...
  showRecording(assigned:string)
  {
    var action = 'show-recording';
    console.log("action:"+action);
    this.openConfirmDialog(assigned, "Are you sure you want to enable user show recording", action);
  }

  hiddenRecording(assigned: string) {
    var action = 'hidden-recording';
    this.openConfirmDialog(assigned, "Are you sure you want to enable user show recording", action);
  }
